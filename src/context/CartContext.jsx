...
export const CartProvider = ({ children }) => {
  ...
  const agregarAlCarrito = (producto) => {
    setCarrito((prev) => {
      const existe = prev.find(p => p.idProductoSucursal === producto.idProductoSucursal);
      if (existe) {
        const nuevaCantidad = existe.cantidad + producto.cantidad;
        if (nuevaCantidad > existe.stock) return prev;
        return prev.map(p => p.idProductoSucursal === producto.idProductoSucursal ? { ...p, cantidad: nuevaCantidad } : p);
      }
